...
1. **Primitive timeout.** `commands.h` declares 8 s; the firmware enforces 15 s.
   If the RPi gives up at 8 s while the STM is still working, the two
   desynchronise and the next reply lands against the wrong command. Pick one
   number and make both sides read it.
...
**Software**
- Task 2 opcodes: `FU`, `FIR`, `FIL`, `FIRO`, `FILO`, `SR`, `SL`
- Motors C and D
