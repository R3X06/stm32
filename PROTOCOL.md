...
**not** been agreed with the RPi owner or tested on the wire. Treat §5, §6 and
the `FAIL,*` rows of §3 as a proposal until that happens — the firmware can
change if the RPi side wants different names or fields.

**Protocol version: 1** — ask for it with `?VER`.
...
|---|---|---|
| `F<n>` | cm | Forward n cm |
| `F0` | — | Forward until an obstacle stops it (ultrasonic, 15 cm) |
| `R<n>` | cm | Reverse n cm |
...
| `?STAT` | `STAT,<state>,<busy>,<imu_ok>,<profile>` | See below |
...
`4` DONE, `5` TIMEOUT. `busy` is 1 while a primitive is running. `imu_ok` is
1 while the gyro is healthy. `profile` is the active arc profile, 0–2.
...
## 7. What the path planner must know
...
**Task 2 movement:** `FU<n>` forward until ultrasound ≤ n cm, `FIR` / `FIL`
forward until the right/left IR loses the wall, `FIRO` / `FILO` forward until
it gains one, `SR` / `SL` diagonal slide.
...
| 9 | Loop a realistic sequence 20× | `rx` on OLED mode 7 stays **0** |
